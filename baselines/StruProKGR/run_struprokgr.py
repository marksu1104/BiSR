...
def run_one(dataset: str, args):
    ...
    log_file = log_root / f"StruProKGR_{dataset}.log"
    fwd_dump = out_dir / "dump_forward.tsv"
    inv_dump = out_dir / "dump_inverse.tsv"
    if dry_run:
        run_strupro(work_dir, dataset, "test.triples", fwd_dump, out_dir, None, args)
        run_strupro(work_dir, dataset, "test_inv.triples", inv_dump, out_dir, None, args)
        return
    ...
    # NOT the legacy STRUPROKGR_METRICS stdout line, which only ranked the
    # gold answer within StruProKGR's own returned candidate list rather than
    # the full entity set and is therefore not a valid Full-entity-set SOTA
    # number under thesis Table 5.
...
def main():
    ...
    parser.add_argument(
        "--work-root",
        default=str(REPO_ROOT / "outputs" / "preprocessed" / "struprokgr"),
    )
