...
def get_train_path(args):
    """Return appropriate train file path. For NELL + non-point models the code historically
    expects *large* variants. If those files are absent, gracefully fallback to existing ones.
    """
    if 'NELL' in args.data_dir:
        if not args.model.startswith('point'):
            large_path = os.path.join(args.data_dir, 'train.dev.large.triples' if args.test else 'train.large.triples')
            # Fallback logic
            if os.path.exists(large_path):
                return large_path
            # Prefer combined or normal variants
            alt1 = os.path.join(args.data_dir, 'train.dev.triples' if args.test else 'train.triples')
            if os.path.exists(alt1):
                return alt1
            return large_path  # will raise later if totally missing
        else:
            return os.path.join(args.data_dir, 'train.dev.triples' if args.test else 'train.triples')
    return os.path.join(args.data_dir, 'train.triples')
