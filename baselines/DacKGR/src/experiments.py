...
def process_data():
    ...
    # NELL large file auto-generation (if expected but missing)
    if 'NELL' in data_dir and not args.model.startswith('point'):
        # infer desired large file names
        desired_large = os.path.join(data_dir, 'train.dev.large.triples' if args.test else 'train.large.triples')
        if (not os.path.exists(desired_large)) and os.path.exists(os.path.join(data_dir, 'train.triples')):
            # create large by copying base train (simple fallback so pipeline won't crash)
            import shutil
            base_src = os.path.join(data_dir, 'train.dev.triples' if args.test and os.path.exists(os.path.join(data_dir, 'train.dev.triples')) else 'train.triples')
            try:
                shutil.copyfile(base_src, desired_large)
                print(f"[process_data] Auto-created missing large train file: {desired_large} (copied from {base_src})")
            except Exception as e:
                print(f"[process_data] Failed to auto-create large file: {e}")
...
def _load_id_maps(entity_index_path, relation_index_path):
    def load_map(path):
        id2name = {}
    ...
    return load_map(entity_index_path), load_map(relation_index_path)

def _export_error_cases_csv(split, data, pred_scores, objects, id2ent, id2rel, out_csv_path, topk=10):
    """
    以與 eval 相同的 filtered 遮蔽計算：
      - rank: 每筆完整 1-based filtered rank（全部實體）
      - topk_pred: 方便檢視的 Top-K 預測（可調整 topk）
    """
    import os, csv
    import numpy as np
    import torch
    ...
    scores_all = torch.from_numpy(pred_scores) if isinstance(pred_scores, np.ndarray) else pred_scores.detach().cpu()

    # 完整 rank（每筆一定有）
    ranks = eval_mod.compute_filtered_ranks(data, scores_all, objects)

    # 取 top-k 以利檢視
    k_eval, pos_list, topk_idx, topk_scores = eval_mod.compute_filtered_topk_and_pos(
        data, scores_all, objects, beam_size=topk
    )

    rows = []
    for i, (h, t, r) in enumerate(data):
        h, t, r = int(h), int(t), int(r)
        target_score = float(scores_all[i, t].item())
        top1_ent_id = int(topk_idx[i][0])
        topk_ids = [int(x) for x in topk_idx[i].tolist()]
        ...
        rows.append({
            ...
            'rel_id':  r,
            'tail_id': t,
            'head': id2ent.get(h, str(h)),
            'relation': id2rel.get(r, str(r)),
            'tail': id2ent.get(t, str(t)),
            'rank': int(ranks[i]),                 # 每筆都有 rank
            'target_score': target_score,
            'top1_ent': id2ent.get(top1_ent_id, str(top1_ent_id)),
            'top1_score': float(topk_scores[i][0]),
            'topk_pred': ';'.join(id2ent.get(e, str(e)) for e in topk_ids),
            'filtered': 1
        })

    os.makedirs(os.path.dirname(out_csv_path), exist_ok=True)
    fieldnames = ['index','split','mode','head','relation','tail','head_id','rel_id','tail_id','rank','target_score','top1_ent','top1_score','topk_pred','filtered']
    ...
    print(f"[{split}] error cases CSV saved: {out_csv_path} (rows={len(rows)}, topk={k_eval})")
...
def export_error_cases(lf):
    ...
    # 重要：原本 _load_id_maps 會把檔案第二欄(實際上是頻次/統計值)誤當成『內部 id』，
    # 導致很多真實的 (head, tail, relation) 內部整數 id 找不到對應名稱而 fallback 成數字字串。
    ...
    print('Test set performance:')
    pred_scores = lf.forward(test_data, verbose=False)
    src.eval.hits_and_ranks(test_data, pred_scores, lf.kg.all_objects, verbose=True, kg=lf.kg)
    src.eval.export_error_cases(test_data, pred_scores, lf.kg.all_objects, os.path.join(lf.model_dir, 'test_error_cases.pkl'))
    _export_error_cases_csv('test', test_data, pred_scores, lf.kg.all_objects, id2ent, id2rel,
                            os.path.join(lf.model_dir, 'test_error_cases.csv'), topk=10)
    # print('Test set performance:')
    # pred_scores = lf.forward(test_data, verbose=False)
    # src.eval.hits_and_ranks(test_data, pred_scores, lf.kg.all_objects, verbose=True, kg=lf.kg)
    # src.eval.export_error_cases(test_data, pred_scores, lf.kg.all_objects, os.path.join(lf.model_dir, 'test_error_cases.pkl'))
