...
class EmbeddingBasedMethod(LFramework):
    ...
    def loss(self, mini_batch):
        ...
        if self.model == '!TransE':
            ...
            # Check if e2 is 2D (batch_size, num_entities) or 1D (batch_size,)
            if e2.dim() == 1:
                # Convert 1D e2 to 2D one-hot format for ConvE model
                batch_size = e2.size(0)
                num_entities = kg.num_entities
                e2_onehot = torch.zeros(batch_size, num_entities).cuda()
                e2_onehot.scatter_(1, e2.unsqueeze(1), 1)
                e2_label = ((1 - self.label_smoothing_epsilon) * e2_onehot) + (self.label_smoothing_epsilon / num_entities)
            else:
                # e2 is already 2D
