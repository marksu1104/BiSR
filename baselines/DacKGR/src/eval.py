...
def compute_mrr_only(examples, scores, all_answers, verbose=False):
    """
    Compute only MRR metric efficiently.
    """
    ...
    for i, example in enumerate(examples):
        ...
        target_score = float(scores_copy[i, e2])
        # mask all false negatives
        scores_copy[i, e2_multi] = 0
        # write back the save prediction
        scores_copy[i, e2] = target_score
    
    # sort and rank
    top_k_scores, top_k_targets = torch.topk(scores_copy, min(scores_copy.size(1), args.beam_size))
...
def hits_and_ranks(examples, scores, all_answers, verbose=False, output=False, kg=None, model_name=None, split_relation=False):
    ...
    for i, example in enumerate(examples):
        ...
        if len(pos) > 0:
            ...
            if pos < 10:
                if output:
                    fr.write('{}\t{}\t{}\n'.format(kg.id2entity[e1], kg.id2entity[e2], kg.id2relation[r]))
                hits_at_10 += 1
                hits_at_10_list[r] += 1
                if pos < 5:
                    hits_at_5 += 1
...
def compute_filtered_topk_and_pos(examples, scores, all_answers, beam_size=None):
    """
    與 hits_and_ranks 相同的遮蔽與 topk 排序邏輯。
    回傳：
      - k: 實際使用的 k
      - pos_list: 每筆 true tail 在 topk 內的位置（找不到為 -1）
      - topk_idx: 每筆的 topk 預測索引 (numpy array, shape [N, k])
      - topk_scores: 每筆的 topk 分數 (numpy array, shape [N, k])
    注意：不會就地修改傳入的 scores。
    """
    import numpy as np
    import torch
    ...
    assert len(examples) == scores.shape[0]
    s = scores.clone()  # 避免修改外部 tensor

    dummy_mask = [DUMMY_ENTITY_ID, NO_OP_ENTITY_ID]
    for i, example in enumerate(examples):
        e1, e2, r = example  # 注意：資料順序為 (head, tail, relation)
        e2_multi = dummy_mask + list(all_answers[e1][r])
        target_score = float(s[i, e2])
        s[i, e2_multi] = 0     # 與 hits_and_ranks 保持一致
        s[i, e2] = target_score

    k = min(s.size(1), args.beam_size if beam_size is None else beam_size)
    top_k_scores, top_k_targets = torch.topk(s, k)
    top_k_targets_np = top_k_targets.cpu().numpy()
    top_k_scores_np = top_k_scores.detach().cpu().numpy()

    pos_list = []
    for i, example in enumerate(examples):
        _, e2, _ = example
        pos = np.where(top_k_targets_np[i] == e2)[0]
        pos_list.append(int(pos[0]) if len(pos) > 0 else -1)

    return k, pos_list, top_k_targets_np, top_k_scores_np    

def compute_filtered_ranks(examples, scores, all_answers):
    """
    回傳每筆樣本的完整 1-based filtered rank（與 hits_and_ranks 相同遮蔽規則）。
    不就地修改傳入的 scores。
    """
    import torch
    s = scores.clone()
    ...
    dummy_mask = [DUMMY_ENTITY_ID, NO_OP_ENTITY_ID]
    for i, example in enumerate(examples):
        e1, e2, r = example  # (head, tail, relation)
        e2_multi = dummy_mask + list(all_answers[e1][r])
        target_score = float(s[i, e2])
        s[i, e2_multi] = 0
        s[i, e2] = target_score
    ...
    for i, example in enumerate(examples):
        _, e2, _ = example
        target = s[i, e2]
        # 嚴格大於的個數 + 1（tie 給相同最小名次）
        rank_i = int((s[i] > target).sum().item()) + 1
        ranks.append(rank_i)
    return ranks
