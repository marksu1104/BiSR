...
### Standardized evaluation output
For easy comparison with other baselines, evaluation now prints one summary line with the prefix `EVAL_STD`.
...
Fields:
- `dir=avg`: bidirectional average (tail + head)
- `filtered=1`: filtered protocol
- `tie_aware=1`: tie-aware ranking
- `full_entity=1`: full-entity ranking
